...
global stayAndWait
stayAndWait = 45#in seconds...how long to stay on the site...the longer, the more messages

global worthlessWords
worthlessWords = ['just','what','will','or','that','for','the', 'i', 'and', 'is', 'in', 'on', 'you', 'a', 'your', 'it', 'this', 'my', 'to']


#list of top 100 coins
global usefulCoinWords
...
def downTrend(text):
	trendDOWN = ['down', 'drop', 'fall', 'sell', 'dead']
	counter = 0;
	for i in text:
		for j in trendDOWN:
			if (i.lower() == j.lower()):
...
def mainFunction():
	...
	#we get words and countser, occurencies of the word
	...
	print ('\x1b[0;37;44m' + str(coinMentions) + '\x1b[0m')
	up=""
	down=""
	for i in range(0,upTrend(messages)):
		up += "+"
	for i in range(0, downTrend(messages)):
		down += "+"

	print("Up trend:   "+'\x1b[6;30;42m' + up + " ("+ str(len(up))+ ")"+'\x1b[0m')
	print("Down trend: "+'\x1b[0;37;41m' + down + " (" + str(len(down)) + ")"+'\x1b[0m')
